...
public class Main {
    ...
    // Projection Matrix
    private final Projection proj = new Projection(45, 1.3333f, 1.0f, 100f);
    private  boolean  orthographicProjection = false;
    private int changedDelay = 0;
    ...
    private void defineProjection() {
		if (Keyboard.isKeyDown(Keyboard.KEY_O) && changedDelay <= 0){
			orthographicProjection = !orthographicProjection;
			changedDelay = 8;
			System.out.printf("Changed camera Projection to ");
			if (orthographicProjection) System.out.printf("orthographic.\n"); else System.out.printf("perspective.\n");
		}
		
		if (changedDelay > 0) changedDelay--;
		
		if (orthographicProjection)
			projMatrix.setTo(proj.orthogonal());
		else
